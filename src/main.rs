...
async fn main() {
    let mut center = BigComplex::from_f64s(-0.5, 0.);
    let max_iterations = 500;
    let bailout2 = 100.;
    let mut pixel_step = 0.005;
    let mut velocity = 1.0;
    let fractal = Fractal::Mandelbrot { power: 2 };
    ...
    loop {
        ...
        let movement = FBig::try_from(velocity * dt)
            .unwrap()
            .with_precision(0)
            .value();
        let old_real = mod_center.real.clone();
        mod_center.real += movement.clone()
        ...
        if moved_x && old_real == mod_center.real {
            mod_center.real = mod_center
                .real
                .with_precision(old_real.precision() + 1)
                .value();
        }

        let old_im = mod_center.im.clone();
        mod_center.im += movement
        ...
        let reference_orbit = ReferenceOrbit::new(&fractal, &center, max_iterations, bailout2);
